...
/// <summary>
/// FileWatcherExtensions 单元测试
/// </summary>
public class FileWatcherExtensionsTests
{
    #region FilterByExtension 测试

    [Fact]
    public async Task FilterByExtension_应该只返回匹配扩展名的事件()
    {
        // Arrange
        var events = new List<FileChangeEvent>
        {
            new() { Path = "/test/file.ts", ChangeType = FileChangeType.Modified, Timestamp = DateTimeOffset.Now },
            new() { Path = "/test/file.js", ChangeType = FileChangeType.Modified, Timestamp = DateTimeOffset.Now },
            new() { Path = "/test/file.css", ChangeType = FileChangeType.Modified, Timestamp = DateTimeOffset.Now },
            new() { Path = "/test/file.md", ChangeType = FileChangeType.Modified, Timestamp = DateTimeOffset.Now }
        };

        // Act
        var filtered = new List<FileChangeEvent>();
        await foreach (var evt in events.ToAsyncEnumerable().FilterByExtension(".ts", ".js"))
        {
            filtered.Add(evt);
        }
        ...
        Assert.All(filtered, e => Assert.True(e.Path.EndsWith(".ts") || e.Path.EndsWith(".js")));
    }

    [Fact]
    public async Task FilterByExtension_扩展名匹配应该不区分大小写()
    {
        // Arrange
        var events = new List<FileChangeEvent>
        {
        ...
        // Act
        var filtered = new List<FileChangeEvent>();
        await foreach (var evt in events.ToAsyncEnumerable().FilterByExtension(".ts"))
        {
            filtered.Add(evt);
        }
    ...
    #region FilterByDirectory 测试

    [Fact]
    public async Task FilterByDirectory_应该只返回指定目录下的事件()
    {
        // Arrange
        var events = new List<FileChangeEvent>
        {
            new() { Path = "/src/components/Button.tsx", ChangeType = FileChangeType.Modified, Timestamp = DateTimeOffset.Now },
            new() { Path = "/src/utils/helper.ts", ChangeType = FileChangeType.Modified, Timestamp = DateTimeOffset.Now },
            new() { Path = "/tests/test.ts", ChangeType = FileChangeType.Modified, Timestamp = DateTimeOffset.Now }
        };

        // Act
        var filtered = new List<FileChangeEvent>();
        await foreach (var evt in events.ToAsyncEnumerable().FilterByDirectory("/src"))
        {
            filtered.Add(evt);
        }
        ...
        // Assert
        Assert.Equal(2, filtered.Count);
        Assert.All(filtered, e => Assert.StartsWith("/src", e.Path));
    }
