...
数据集：MDN Web Docs 全量（14621 页，宏行剔除）+ kubernetes/website
英文文档（349 页）。首次测量报 Flint 6007ms vs Hugo 6763ms（0.89x），
**该结论已撤回**：产物审计发现 Hugo 侧只渲染了 362 个 html——MDN 的
14621 个 index.md 在 Hugo 的 content 结构下未被渲染为页面（生成结构
与 Hugo 的 leaf bundle/unsafe HTML 语义不匹配，WARN Raw HTML omitted
大面积出现），"快 11%"是 Hugo 侧未完成渲染造成的假象。

**有效结论修正为**：万页合成 0.92-0.95x 与官方 3978 页 0.98x 两个
口径成立；MDN 语料需按 Hugo 的 content 结构规范重生成后才能进入
对比。当前证据链下 Flint 与 Hugo 持平（±2-5%）。

教训：**跨引擎对比必须做产物数量对称性审计**（本次审计由"数据集
是否公平"的追问触发）——只看耗时不看产物会得出错误结论。

### 【已修正重测】MDN+k8s 公平对比（2026-09-08 第二次）

根因修复：转换器两处修正——嵌套 index.md 改扁平化命名
（mdn-web-api-gamepad.md，避免 Hugo leaf bundle 吞子页面与 Flint
树 key 冲突）、剥离 MDN 原文件自带 front matter（消除双重 FM）。
产物对称性确认：Hugo 14937 vs Flint 14940 个 html。

| 引擎 | 14974 页真实内容 | 单页 | vs 万页合成 |
| ---- | ------------------ | ------ | ----------- |
| Hugo v0.165.0 | 9056ms | 0.605ms | — |
| **Flint 0.1.0** | **6926ms** | **0.463ms** | **0.76x，快 24%** |

**MDN 页面（简单标量 FM + HTML/代码密集正文）下 Flint 快 24%**——
FM 快速路径全量生效（MDN 页 FM 全为简单标量）+ Markdig 与
Goldmark 的 HTML 块处理差异。结论更新：**两个真实内容口径
（3978 页 0.98x、14974 页 0.76x）与万页合成（0.92-0.95x）下
Flint 全部持平或更优**。


## 生产环境标准复测（2026-09-08，Release+AOT，三语料×双引擎×3 次）

Flint 按 AOT 原生发布（对齐 Hugo 原生二进制对等口径），
同轮同机同语料对比（中位数）：

| 语料 | Hugo | Flint AOT | 比值 |
| ---- | ---- | --------- | ---- |
| 万页合成 10000 页 | 5290ms | 3912ms | 0.74x |
| 官方基准 3978 页 | 1326ms | 365ms | 0.28x |
| MDN 14621 页 | 9393ms | 6147ms | 0.65x |

进程内性能套件（Release，11/11 全绿）：小型 97ms、中型 422ms、
大型 826ms、超大型 6651ms、增量加速比 5.68x、模板渲染 19.2ms、
Markdown 201k 文件/秒。

**注意**：轮间机器状态漂移显著（同语料历史轮差可达 ±30%），
跨轮数字仅作参考，同轮内对比有效。

## 主题复杂度阶梯对照实测（2026-09-08，1000 页 × L1/L2/L3 × 双引擎）
