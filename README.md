...
- **🚀 极速构建**：万页站点 3.5 秒、MDN 14.6k 页 7.1 秒（同窗均比 Hugo 快 17~19%），增量 75ms，Markdown 解析 238k 文件/秒
- **🧠 内存可控**：万页构建内存峰值 530MB，14.6k 页技术文档语料 1494MB——低于 Hugo 同窗（1531MB）
...
- **绝对值不可跨环境直比**：同一 Hugo 冻结二进制同日从 4530~5540ms 漂移到
  ...
  （`scripts/perf-baseline.json`，复跑 9/9 PASS）；相对 09-08 epoch 的 3 项稳定
  劣化（增量 +55% / 复杂主题 +43.6% / 100 页 +31.7%）记于该文件
  `known_regressions` 持续跟踪，不随重录消失。
