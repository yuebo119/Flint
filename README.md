...
- **🚀 极速构建**：万页站点 5.1 秒，增量 69ms，Markdown 解析 222k 文件/秒
...
|------|------|---------|------:|--------------:|:----:|
| 万页合成 | 10,007 | 同构 lorem | 6927ms | **5081ms** | **0.73x** |
...
- **比值已回到报告水平**：写入相目录创建优化（段级缓存 + Win32 单段原语）
  后两轮 0.77x / 0.73x，与 REPORT（2026-09-08/09）的 0.74x 持平；
  优化贡献写入相中位数 -42%（4417 → 2576ms），为全构建最大单一矿脉。
- **绝对值含环境税**：同一 Hugo 冻结二进制同日从 4530~5540ms 漂移到
  6927~7339ms（+30%，机器负载 25~48%）——绝对值与报告不可直接比，
  比值经 Hugo 同窗自对照已做负载归一化。
- **未决项（不影响上述比值）**：AOT 产物的扫描/渲染相慢于同源码 JIT 对照
  ...
  剩余嫌疑为防护的进程级策略（腾讯电脑管家 RTP 标记 NOT_STOPPABLE 无法
  关闭，反向验证未能完成）——挂起待查，待可关闭时补一轮 AOT vs JIT 判定。
...
`scripts/gotmpl2scriban.py <hugo-theme-dir> <flint-theme-dir>` 自动转换
